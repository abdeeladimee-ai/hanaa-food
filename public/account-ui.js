(() => {
  ...
  const ensureStyles = () => {
    ...
    style.textContent = `
      ...
      .hanaa-account-page .hanaa-account-title { display:block;font-size:16px;font-weight:950; }
      .hanaa-account-page .hanaa-account-subtitle {
        display:block;margin-top:4px;font-size:12.5px;line-height:1.3;color:#9a8385;font-weight:650;
      }
  ...
  const buttonConfig = (label) => {
    if (/Mes commandes/i.test(label)) return { icon:"🧾", title:"Mes commandes", subtitle:"Suis tes commandes et retrouve ton historique" };
    if (/Mes favoris/i.test(label)) return { icon:"♡", title:"Mes favoris", subtitle:"Retrouve rapidement tes plats préférés" };
    if (/Mon profil/i.test(label)) return { icon:"♙", title:"Mon profil", subtitle:"Modifie ton nom et ton numéro de téléphone" };
    return null;
  };
  ...
  const decorateAccount = () => {
    ...
    const safePhone = (phone || "Ton espace personnel Hanaa Food").replace(/[<>]/g, "");
    const signature = `${initial}|${safeName}|${safePhone}`;
    ...
    if (card.dataset.signature !== signature) {
      ...
      card.innerHTML = `
        ...
        <div class="hanaa-account-copy">
          <div class="hanaa-account-name">${safeName}</div>
          <div class="hanaa-account-phone">${safePhone}</div>
        </div>
      `;
    }

    [...list.querySelectorAll(":scope > button")].forEach((button) => {
      if (button.dataset.hanaaAccountDecorated === "1") return;
      const config = buttonConfig(clean(button.textContent));
      if (!config) return;
  ...
  const scheduleDecorate = () => {
    if (scheduled) return;
    scheduled = true;
    window.requestAnimationFrame(decorateAccount);
  };
  ...
  document.addEventListener("submit", () => window.setTimeout(scheduleDecorate, 120), true);
  scheduleDecorate();
})();
